...
def test_eval_heu():
    from agents.agent_minimax.minimax import eval_heu, assign_weight
    ...
    game, val_board = assign_weight(board, 3, PLAYER1, board_values=BOARD_VALUES)
    break_y, minimizing_case= eval_heu(cumulative_heuristic, val_board, i, idx, game, node_type=np.array([1]))

    assert minimizing_case == cumulative_heuristic - val_board # Proving if cumulative works
    assert idx == [i]

    break_y, maximizing_case = eval_heu(cumulative_heuristic, val_board, i, idx, game, node_type=np.array([-1]))
    assert idx == [i,i]
    ...
    break_y, column_full = eval_heu(cumulative_heuristic, val_board, i, idx, game, node_type=np.array([-1]))
...
def test_max_child():
    from agents.agent_minimax.minimax import max_child, minmax_tree
...
def test_min_child():
    from agents.agent_minimax.minimax import min_child, minmax_tree
